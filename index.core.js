...
window.open = (url, target, feature) => {
    if (url.indexOf("player.html") >= 0) {
        window.videoFrame = document.createElement("iframe");
        ...
        window.clsBtn = document.createElement("img");
...
const init = async () => {
    ...
    window.open("./player.html?video/logo.webm")
};
init();